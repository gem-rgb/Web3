...
function App() {
  ...
  const [gatewayHealth, setGatewayHealth] = useState<'healthy' | 'degraded' | 'offline' | 'unknown'>('unknown')
  ...
  useEffect(() => {
    ...
    const loadGatewayState = async () => {
      try {
        ...
        if (!cancelled) {
          setGatewayHealth(healthJson.status === 'ok' ? 'healthy' : 'degraded')
    ...
    <div className="dashboard-shell">
      ...
      <main className="dashboard">
        ...
        <section className="gateway-strip">
          ...
          <span><strong>{gatewayMetrics.requests}</strong> order evaluations</span>
          <span><strong>{gatewayMetrics.approved}</strong> approved</span>
          <span><strong>{gatewayMetrics.rejected}</strong> rejected</span>
          <span><strong>{gatewayMetrics.throttled}</strong> throttled</span>
        </section>
